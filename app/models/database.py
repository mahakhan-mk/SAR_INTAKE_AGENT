...
class AssessmentResponse(Base):
    ...
    answer_value: Mapped[dict[str, object] | None] = mapped_column(JSONB_TYPE, nullable=True)
...
class QuestionRiskResult(Base):
    ...
    question_definition_id: Mapped[uuid.UUID] = mapped_column(
        UUID_TYPE,
        ForeignKey("question_definitions.id"),
        nullable=False,
        index=True,
    )
    selected_option_id: Mapped[uuid.UUID | None] = mapped_column(UUID_TYPE, ForeignKey("question_options.id"), nullable=True)
    question_text: Mapped[str] = mapped_column(Text, nullable=False)
    risk_domain: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
...
Index("ix_question_options_question_order", QuestionOption.question_id, QuestionOption.display_order)
Index("ix_assessment_responses_assessment_question", AssessmentResponse.assessment_id, AssessmentResponse.question_id)
